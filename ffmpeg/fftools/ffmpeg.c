...
#define show_help_default ffmpeg_show_help_default
                         ...
                         AVFrame *next_picture)
{
    ...
    for (i = 0; i < nb_frames; i++)
    {
        ...
        else if (ost->forced_keyframes && !strncmp(ost->forced_keyframes, "source", 6) && in_picture->key_frame == 1 && !i)
        {
            forced_keyframe = 1;
        }
        else if (ost->forced_keyframes && !strncmp(ost->forced_keyframes, "source_no_drop", 14) && !i)
        {
...
static int init_output_stream_encode(OutputStream *ost, AVFrame *frame)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (av_q2d(enc_ctx->time_base) < 0.001 && ost->vsync_method != VSYNC_PASSTHROUGH && (ost->vsync_method == VSYNC_CFR || ost->vsync_method == VSYNC_VSCFR || (ost->vsync_method == VSYNC_AUTO && !(of->format->flags & AVFMT_VARIABLE_FPS))))
        {
...
static OutputStream *choose_output(void)
{
    ...
    {
        OutputStream *ost = output_streams[i];
        int64_t opts = ost->last_mux_dts == AV_NOPTS_VALUE ? INT64_MIN : av_rescale_q(ost->last_mux_dts, ost->st->time_base, AV_TIME_BASE_Q);
...
static int get_input_packet(InputFile *f, AVPacket **pkt)
{
    ...
    {
        int i;
        int64_t file_start = copy_ts * ((f->ctx->start_time != AV_NOPTS_VALUE ? f->ctx->start_time * !start_at_zero : 0) +
